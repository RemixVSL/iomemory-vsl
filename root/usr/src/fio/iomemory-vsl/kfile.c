...
/**
* @brief linux: calls create_proc_read_entry(..)
*/
fusion_proc_dir_entry * noinline kfio_create_proc_read_entry(const char *name,
    fio_mode_t mode, fusion_proc_dir_entry *base, fusion_read_proc_t *read_proc, void *data)
{
    return create_proc_read_entry(name, mode, (struct proc_dir_entry *) base,
        (read_proc_t *) read_proc, data);
}

/**
* @brief linux: calls create_proc_write_entry(..)
*/
fusion_proc_dir_entry * noinline kfio_create_proc_write_entry(const char *name,
    fio_mode_t mode, fusion_proc_dir_entry *base, fusion_write_proc_t *write_proc, void *data)
{
    struct proc_dir_entry *entry = create_proc_entry(name, mode, base);
    if (entry) {
        entry->write_proc = (write_proc_t *) write_proc;
        entry->data = data;
    }
    return entry;
}
...
{
    struct proc_dir_entry *entry = create_proc_entry(name, mode, base);
    kassert(sizeof (struct file_operations) <= sizeof (fusion_file_operations_t) );
    if (entry) {
...
/**
* @brief linux: sets entry->proc_read
*/
void noinline kfio_set_write_proc_dir_entry(fusion_proc_dir_entry *entry, fusion_write_proc_t *write_proc)
{
    ((struct proc_dir_entry *) entry)->write_proc = (write_proc_t *) write_proc;
}

/**
* @brief linux: sets entry->owner
*/
void noinline kfio_set_owner_dir_entry(fusion_proc_dir_entry *entry, void *owner)
{
#if KFIOC_OWNER_IN_STRUCT_PROC_DIR_ENTRY
    ((struct proc_dir_entry *) entry)->owner = owner;
#endif
}
