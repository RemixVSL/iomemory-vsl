 ...
 * @return pointer to vmapped memory on sucess else NULL.
 */
...
static vmk_MachPage vmklnx_alloc_page(gfp_t flags)
{
     ...
     */
        ...
        {
            errprint("Failure to increase mem pool to %u, g_pages_used=%lld\n",
                 pages_to_reserve, g_pages_used);
        }
        else 
        {
            g_pages_reserved = pages_to_reserve;
    ...
    retval = vmk_MemPoolAlloc(g_vmklnx_mem_pool, &alloc_props, size / PAGE_SIZE,
                              can_wait, &page);
    if (retval == VMK_MEM_ADMIT_FAILED)
    {
...
vmk_VirtAddr vmklnx_page_address(vmk_MachPage page)
{
    vmk_MachAddr     maddr;
    vmk_VirtAddr     vaddr;
    VMK_ReturnStatus retval;
 ...
 *  @parameter pdev - pointer to device handle
 *      Solaris  (struct _kfio_solaris_pci_dev *)
 *  @parameter size - size of allocation
 *  @parameter dma_struct - member .phys_addr of struct will be set to the 
...
/**
 * @brief frees memory allocated by @f kfio_dma_alloc_coherent.
 *
 */
