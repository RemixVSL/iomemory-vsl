...
static int kfio_info_os_proc_read(char *page, char **start, fio_off_t off, int count, int *eof, void *data)
{
    kfio_info_node_t *nodep = data;
    kfio_info_data_t *dbh;
    int rc;

    rc = kfio_info_alloc_data_handle(nodep, page, count, &dbh);
    if (rc != 0)
    {
        return rc;
    }
    ...
    if (rc == 0)
    {
        *eof = kfio_info_data_get_eof(dbh);
        rc  = kfio_info_data_size_valid(dbh);

        if (*eof == 0)
        {
            /* Data will be placed at the beginning of the buffer. */
            *start = page;
        }
    }
...
int kfio_info_linux_seq_open(fusion_inode *inode, fusion_file *file)
{
...
static void kfio_info_os_init(void)
{
    ...
    kfio_set_file_ops_open_handler   (&kfio_info_seqf_fops, kfio_info_linux_seq_open);
...
int kfio_info_os_create_node(kfio_info_node_t *parent, kfio_info_node_t *nodep)
{
    ...
    {
        node_entry = kfio_create_proc_read_entry(node_name, node_mode, parent_dir,
                                                 kfio_info_os_proc_read, nodep);
    }
...
EXPORT_SYMBOL(kfio_info_create_seqf);
EXPORT_SYMBOL(kfio_info_create_dir);
EXPORT_SYMBOL(kfio_info_create_text);
EXPORT_SYMBOL(kfio_info_remove_node);
EXPORT_SYMBOL(kfio_info_handle_cmd);
EXPORT_SYMBOL(kfio_info_create_proc);
EXPORT_SYMBOL(kfio_info_printf);
EXPORT_SYMBOL(fusion_info_root);
